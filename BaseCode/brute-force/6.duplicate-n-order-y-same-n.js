// note: 문제
// note:   입력 n개를 받을떄 m개를 중복 없이, 순서 있게 선택하기, 같은 결과를 한번만 나오게 하기
// note:     ex) n: 4, m: 3
// note:     ex) [4, 9, 3, 1]
...
let cands;
let selecteds = [];
let isSelecteds = Array(n).fill(false);
let results = [];
let resultString = '';
...
function recursion(k) {
  if (k === m) { // note: 모든 경우의 수를 구했다면?
    ...
    results.push(answerString);
  } else { // note: k번째로는 무엇을 선택할 것인가?
    let lastCand = 0;
    for (let cand = 0; cand < n; cand++) {
      if (isSelecteds[cand]) continue; // 하나만 있는 숫자는 한번만 선택하게 하려고 (중복 없이)
