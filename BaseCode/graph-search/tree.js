...
// todo:   3. 대부분 DFS 를 사용하는 것이 좋다. 트리는 BFS로 푼다라고 외우자!!
...
// note:   2. 트리는 BFS로도 충분히 깊이를 구할 수 있다.
// note:   3. 트리에서 사이클이 존재하는지 판단하려면 Path를 계산해야한다. 
// note:      (조건. Path를 계산하려면 단방향 그래프, 방문처리 계산하지 않아야함)
// note:      마지막에 Path에 중복되게 방문한 노드가 있으면 사이클이 존재하는 것이다.