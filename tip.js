...
// note: 9. 패캠에서 알려주는 코딩 테스트 공부하기에 좋은 습관 (순크시공함느테)
// note:    1. 순: 문제를 꼼꼼히 읽고 순서대로 풀자.
// note:    2. 크: 변수 값의 최대값, 최소값을 계산해서 int를 쓸지, long을 쓸지 정해야한다.
// note:    3. 시.공: 시간 복잡도, 공간 복잡도를 계산해서 풀이할 가치가 있는지 판단한다.
// note:    5. 느: 느린 풀이법부터 빠른 풀이법을 생각한다. 왜냐하면 실제로 그렇게 풀이법을 생각하기도하고 아니면 부분 점수라도 받기 위해서이다.
// note:    4. 함: 풀이를 함수화하여 코드를 읽기 좋게, 디버깅을 쉽게, 케이스를 나누어 생각할 수 있게 만든다.
// note:    6. 테: 테스트 케이스를 작성해서 문제를 푼다.
      
// note: 10. 코딩의 기본은 '초기값', '인풋', '아웃풋' 체크하는것이다.

// note: 11. 디버깅은 무조건 부분적으로 나눠서 디버깅해야합니다. 

// note: 12. 연습은 실전보다 어렵게. 
// note:       카카오 경력 코테로는 골드4~5정도로 문제가 나오는거 같다.
// note:       하지만 코테를 골드1정도까지만 풀수 있으면 정도로 노력해두면 시험 볼 때 당황하지 않고 여유롭게 문제를 풀 수 있을거같다.
// note:       여유롭게 풀어야 나중에 리펙토링도 할 수 있다.
...
// note: 알고리즘 공부: 코테 준비가 완전 처음이라면 알고리즘 공부부터 해야함
// note: 문제풀때 시간 제한: 알고리즘 공부가 끝나고 문제푸는것이 어느정도 익숙해지면 문제 풀때마다 시간제한하자 (코테보기 1~2달전)
// note: 정답코드 확인: 정답 코드, 좋은 코드 꼭 확인하기
...
// fixme: 나의 디버깅 실력을 빨리 올리는 방법
// note: 디버깅을하고 내 디버깅 실력을 피드백한다.
// note:   문제를 풀고 못풀면 답을 확인해서 문제를 푼다. 
// note:   그 후에 "내가 왜 못풀었지?" 어떤 것이 부족했는지 피드백한다. 
// note:   그리고 그 잘못한 점 만큼 푸시업을 진행한다.
// note:     예) 이번 문제를 풀면서 모든 코드를 다 읽고 디버깅을 하지 않았음 - 반정도만 읽고 생각을 포기함 => 푸시업 1회 
// note:     에) 엣지 테스트 케이스 찾았는데 그것이 정답이 잘 나오고 있다고 착각함 => 푸시업 1회